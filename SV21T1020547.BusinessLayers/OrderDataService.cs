...
namespace SV21T1020547.BusinessLayers
{
    public static class OrderDataService
    {
        ...
        /// </summary>
        public static int InitOrder(int employeeID, int customerID, string deliveryProvince,
            string deliveryAddress, IEnumerable<OrderDetail> details)
        {
            if (details.Count() == 0)
                return 0;
            Order data = new()
        ...
        public static Dictionary<int, String> SelectStatus()
        {
            Dictionary<int, String> list = new()
            {
                { Constants.ORDER_INIT, "Đơn hàng mới. Đang chờ duyệt" },
                { Constants.ORDER_ACCEPTED, "Đơn đã chấp nhận. Đang chờ chuyển hàng" },
                { Constants.ORDER_SHIPPING, "Đơn hàng đang được giao" },
                { Constants.ORDER_FINISHED, "Đơn hàng đã hoàn tất" },
                { Constants.ORDER_CANCEL, "Đơn hàng đã bị hủy" },
